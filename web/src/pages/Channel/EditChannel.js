...
const EditChannel = () => {
  ...
  const submit = async () => {
    ...
    if (localInputs.backup_url && localInputs.backup_url.endsWith('/')) {
      localInputs.backup_url = localInputs.backup_url.slice(0, localInputs.backup_url.length - 1);
    }
            ...
            inputs.type !== 3 && inputs.type !== 8 && inputs.type !== 22 && (
              <Form.Field>
                <Form.Input
                  label='代理'
                  name='base_url'
                  placeholder={'此项可选，用于通过代理站来进行 API 调用，请输入代理站地址，格式为：https://domain1.com;https://domain1.com'}
                  onChange={handleInputChange}
                  value={inputs.base_url}
                  autoComplete='new-password'
                />
