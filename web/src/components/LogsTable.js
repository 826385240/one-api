...
const LogsTable = () => {
  ...
  const retryPost = async (id) => {
    ...
    let localEndTimestamp = Date.parse(end_timestamp) / 1000;
    if (isAdminUser) {
      url = `/api/log/retry?logid=${id}&type=${logType}&username=${username}&token_name=${token_name}&model_name=${model_name}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}&channel=${channel}`;
    } else {
      url = `/api/log/retry?logid=${id}&type=${logType}&token_name=${token_name}&model_name=${model_name}&start_timestamp=${localStartTimestamp}&end_timestamp=${localEndTimestamp}`;
    }
    const res = await API.get(url);
    const { success, message, data } = res.data;
    if (success) {
              ...
              .map((log, idx) => {
                ...
                return (
                  <Table.Row key={log.id}>
                    ...
                    <Table.Cell>{log.reply}</Table.Cell>
                    <Button onClick={() => retryPost(log.id)}>
                      重放
                    </Button>
                  </Table.Row>
                );
