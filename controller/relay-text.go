...
func relayTextHelper(c *gin.Context, relayMode int) *OpenAIErrorWithStatusCode {
	ginContext := common.SaveContextToString(c)
	...
	if apiType != APITypeXunfei { // cause xunfei use websocket
		io.Copy(&copyBody, requestBody)
		requestBody = bytes.NewReader(copyBody.Bytes())
		...
		if apiType == APITypeOpenAI && channelType == common.ChannelTypeAzure && UseAzureBackup {
			// azure切换备用url
			if c.GetString("backup_url") != "" {
				backupURL, _ := url.Parse(c.GetString("backup_url"))
				transport := &http.Transport{
					Proxy: http.ProxyURL(backupURL),
				}
	...
	defer func(ctx context.Context) {
		...
		go func() {
			...
			if quota != 0 {
				...
				var req Request
				bodyBytes, _ := io.ReadAll(&copyBody)
				json.Unmarshal(bodyBytes, &req)
				index := len(req.Messages) - 1
				// 回复
				model.RecordConsumeLog(ctx, userId, channelId, promptTokens, completionTokens, textRequest.Model, tokenName, quota, logContent, req.Messages[index].Content.(string), responseText, relayMode, ginContext)
	...
	case APITypeOpenAI:
		if isStream {
			...
			var err *OpenAIErrorWithStatusCode
			var usage *Usage
			err, usage, responseText = openaiHandler(c, resp, promptTokens, textRequest.Model)
			if err != nil {
				return err
			}
			if usage != nil {
