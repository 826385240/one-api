...
func RetryUserLog(c *gin.Context) {
	id, _ := strconv.Atoi(c.Query("logid"))
	logs, err := model.GetUserLogsById(id)
	if err != nil {
		c.JSON(http.StatusOK, gin.H{
			"success": false,
			"message": err.Error(),
	...
	for _, log := range logs {
		resp, err := common.NewRequestFromContextString(log.GinContext)
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "Failed to make a new request"})
			return
		}
